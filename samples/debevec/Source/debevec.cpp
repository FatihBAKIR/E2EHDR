...
cv::Mat Debevec::Sample(const Exposure& exp)
{
...
float Debevec::weight(int pixelVal)
{
...
void Debevec::LoadImage(gsl::span<char> buffer, int w, int h)
{
	Debevec::Exposure exposure;
...
vector<int> Debevec::SampleSelection(const Exposure& image)
{
	int w = image.w;
	int h = image.h;
